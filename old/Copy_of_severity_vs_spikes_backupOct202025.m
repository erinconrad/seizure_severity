...
%% ===== Per-patient metrics from spike table S =====
% Already have per-patient mean spike rate as Sg:
%   Sg: Patient, MeanTotalSpikes, MeanSpikeRate_perHour

% --- NEW: Per-patient longest spike burst (max across EEGs) ---
...
% Merge spike-rate + longest-burst into one per-patient table
Spp = outerjoin(Sg, Sb, 'Keys','Patient', 'MergeKeys', true);
% (Optional) If a patient has no bursts column for some reason, fill with 0
Spp.MaxLongestBurst_sec(~isfinite(Spp.MaxLongestBurst_sec)) = 0;

%% ===== Build per-patient seizure frequency table Rg (you already have this) =====
% Rg: columns Patient, MeanSzFreq
% PerPatType: columns Patient, EpilepsyType
% (Your code above constructs Rg and PerPatType, and filters to valid patients.)

% Attach epilepsy type
Rg = innerjoin(Rg, PerPatType(~isBad, :), 'Keys','Patient');

%% ===== Join per-patient tables (now only valid epilepsy_type patients remain) =====
% Build per-patient spike metrics
if ~ismember('Longest_Spike_Duration_sec', S.Properties.VariableNames)
    error(['Column "Longest_Spike_Duration_sec" not found in %s. ', ...
           'Re-run the counting script that writes this column.'], outCsv);
end
...
% Ensure Rg has EpilepsyType attached
Rg = innerjoin(Rg, PerPatType(~isBad, :), 'Keys','Patient');
...
% If an exact 'EpilepsyType' exists, use it as the base; otherwise rename the first match to 'EpilepsyType'
if any(strcmp(etCols, 'EpilepsyType'))
    baseName = 'EpilepsyType';
else
    baseName = etCols{1};
    P = renamevars(P, baseName, 'EpilepsyType');
    baseName = 'EpilepsyType';
end

% Coalesce values from any other EpilepsyType* columns into the base, then drop them
others = setdiff(etCols, baseName, 'stable');

% Ensure the base is string
if ~isstring(P.EpilepsyType)
    P.EpilepsyType = string(P.EpilepsyType);
end
...
for ii = 1:numel(others)
    ...
    v = strtrim(v);
    fillIdx = (ismissing(P.EpilepsyType) | strlength(P.EpilepsyType)==0) & ...
              (~ismissing(v) & strlength(v)>0);
    P.EpilepsyType(fillIdx) = v(fillIdx);
end

if ~isempty(others)
    P = removevars(P, others);
end

% ---- Normalize labels exactly to {'Focal','General'} and drop the rest ----
et = lower(strtrim(P.EpilepsyType));
isGeneral = contains(et, "general");  % 'generalized', 'general', etc.
...
% ---- Ensure EpilepsyType column exists with the exact name ----
etCols = P.Properties.VariableNames(startsWith(P.Properties.VariableNames, 'EpilepsyType', 'IgnoreCase', true));
if isempty(etCols)
    error('EpilepsyType column not found after joins. Available columns: %s', strjoin(P.Properties.VariableNames, ', '));
elseif ~strcmp(etCols{1}, 'EpilepsyType')
    P = renamevars(P, etCols{1}, 'EpilepsyType');
end

% Clean types and rows
if ~isstring(P.EpilepsyType), P.EpilepsyType = string(P.EpilepsyType); end

% --- Normalize epilepsy labels to exactly {'Focal','General'} ---
et = lower(strtrim(P.EpilepsyType));
isGeneral = contains(et, "general");    % matches 'Generalized', 'General epilepsy', etc.
isFocal   = contains(et, "focal");
% If neither matched (e.g., 'Mixed'), mark as missing so we drop it
normType = strings(size(et));
normType(isGeneral) = "General";
normType(isFocal)   = "Focal";
P.EpilepsyType = normType;

% Drop rows with missing/invalid predictors or outcomes or labels
P = P(~isnan(P.MeanSzFreq) & isfinite(P.MeanSpikeRate_perHour) & ...
      isfinite(P.MaxLongestBurst_sec) & strlength(P.EpilepsyType)>0, :);
...
%% ===== Overall correlation: Longest burst vs seizure frequency =====
xb = P.MaxLongestBurst_sec;      % seconds
yb = P.MeanSzFreq;               % seizures/month (as parsed earlier)

mask = isfinite(xb) & isfinite(yb);
xb = xb(mask); yb = yb(mask);

% Spearman (rank-based) on raw scales
...
% Log-safe transforms for plotting (handles zeros)
...
%% ===== Stratified correlations: Focal vs General =====
wantTypes = ["Focal","General"]; % <- make sure these match your EpilepsyType labels
