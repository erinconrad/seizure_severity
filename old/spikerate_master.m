%% spikerate_master_clearNames_outptElecs.m
...
%   * Build patient typing + seizure frequency once from the (possibly
%     filtered) report.
%   * Keep a FULL view (no only_amb restriction) for Amb vs Routine
%     comparisons.
...
% Normalized acquired_on string (for outpatient classification)
if hasAcqCol
    acqStr_report = lower(strtrim(string(ReportTable.acquired_on)));
else
    acqStr_report = strings(height(ReportTable),1);
end
...
%% ======================= QUICK MODALITY COMPARISONS (FULL VIEW) =======================
...
% Required jay_* spike columns
jayCols = {'jay_focal_epi','jay_multifocal_epi','jay_gen_epi'};
missingJay = setdiff(jayCols, ReportForKeptSessions.Properties.VariableNames);
if ~isempty(missingJay)
    error('Report table for kept sessions lacks required jay_* spike columns: %s', ...
        strjoin(missingJay, ', '));
end
...
rawMain = lower(strtrim(string(ReportForKeptSessions.(reportColName))));
isMainPresent = ismember(rawMain, tokPresent);
isMainAbsent  = ismember(rawMain, tokAbsent);
...
isF_P = ismember(rawF, tokPresent);
isF_A = ismember(rawF, tokAbsent);
isM_P = ismember(rawM, tokPresent);
isM_A = ismember(rawM, tokAbsent);
isG_P = ismember(rawG, tokPresent);
isG_A = ismember(rawG, tokAbsent);
...
if any(discordMask)
    badRows = find(discordMask);
    % Show a few problematic rows to help debug
    nShow = min(5, numel(badRows));
    fprintf(2, '\n*** Discordant spike reporting detected in %d rows (showing first %d):\n', ...
        numel(badRows), nShow);
    for k = 1:nShow
        r = badRows(k);
        fprintf(2, '  Row %d: patient_id=%g, session_number=%g, %s="%s", F=%s, M=%s, G=%s\n', ...
            r, ...
            double(ReportForKeptSessions.patient_id(r)), ...
            double(ReportForKeptSessions.session_number(r)), ...
            reportColName, string(rawMain(r)), ...
            string(rawF(r)), string(rawM(r)), string(rawG(r)));
    end
    error(['Discordant spike presence between %s and jay_* columns. ' ...
           'See stderr output above for examples.'], reportColName);
end
...
% ---- Panel A: Epilepsy (any) vs NESD (patient-level means) ----
...
p_rankSum_A = NaN;
if n_ep >= 3 && n_nes >= 3
    p_rankSum_A = ranksum(x_ep, x_nes, 'method','approx');
end
...
% ---- Panel B: General vs Temporal vs Frontal (patient-level) ----
...
if ~isempty(Y_C)
    ...
    try
        ...
        scatter(axC, double(categorical(G_C)) + 0.05*randn(size(Y_C)), Y_C, 14, ...
...
if isfinite(p_rankSum_C)
    ...
    text(axC, 0.5,0.97,'Insufficient N','Units','normalized', ...
...
if ~isempty(Y_A)
    ...
    try
        ...
        scatter(axA, double(categorical(G_A)) + 0.05*randn(size(Y_A)), Y_A, 14, ...
            'filled','MarkerFaceAlpha',0.4);
    end
end
...
if isfinite(p_rankSum_A)
    ...
    text(axA, 0.5,0.97,'Insufficient N','Units','normalized', ...
        'HorizontalAlignment','center','FontSize',20);
end
set(axA,'FontSize',20);
...
if ~isempty(Y_B)
    ...
    try
        swarmchart(axB, SubtypeSubsetTable.EpiType4, Y_B, 18, 'filled','MarkerFaceAlpha',0.45);
    catch
...
for j = 1:height(R)
    ...
    if strlength(s)>0 && s~="[]" && s~=""
        ...
        try
            ...
            if ~isempty(nums)
                v = double(str2double(string(nums(:))));
            end
        end
    end
