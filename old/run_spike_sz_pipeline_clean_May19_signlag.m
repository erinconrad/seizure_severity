...
write_results_html(resultsHtml, Views, SzFreqPerPatient, Fig1Stats, ...
    ...
    Views.ReportForKeptSessions, MMR, Vuniq);
...
%% ---- MODEL FORMULAS ----
% Primary model: signed lag + interaction with spike rate
formula_M1 = ['HasSz_bin ~ ' ...
    'LogSpikesPerHour * SignedLag_years + ' ...
    'EpiType3_cat + (1|PatientID)'];

% Alternate model: no interaction term
...
disp_names(disp_names=="SignedLag_years")                  = "EEG-visit lag, signed (years)";
disp_names(disp_names=="LogSpikesPerHour:SignedLag_years") = "Spike rate x signed lag";
...
b_lag       = beta_pred(bnames=="SignedLag_years");
b_int       = beta_pred(bnames=="LogSpikesPerHour:SignedLag_years");
...
for k = 1:numel(lag_vals)
    lag_k  = lag_vals(k);
    or_k   = exp(b_spike + b_int*lag_k);
    eta_k  = b_intercept + b_spike*log(x_anno+EPS_SPIKE) + b_lag*lag_k + ...
             b_int*log(x_anno+EPS_SPIKE)*lag_k;
...
title(lg,'EEG-visit lag (visit after EEG)');
...
function write_results_html(outPath, Views, SzFreqPerPatient, Fig1Stats, ...
    ...
    SpearmanResults_S1, rs_all_S1, p_all_S1, n_all_S1, ...
    rho_lo_S1, rho_hi_S1, subtype_ci_S1, ...
    ReportForKeptSessions, MMR, Vuniq)
