...
write_results_html(resultsHtml, Views, SzFreqPerPatient, ...
    Fig1Stats, ...
    SpearmanResults_main, rs_all_main, p_all_main, n_all_main, rho_lo_main, rho_hi_main, subtype_ci_main, ...
    Views.ReportForKeptSessions, MMR, MMR.LRT3_p, MMR.LRT2_p);
...
function write_results_html(outPath, Views, SzFreqPerPatient, ...
    Fig1Stats, ...
    SpearmanResults_main, rs_all_main, p_all_main, n_all_main, rho_lo_main, rho_hi_main, subtype_ci_main, ...
    ReportForKeptSessions, MMR, lrt3_p, lrt2_p)
...
fprintf(fid, ['<p>To formally test the association between spike rate and seizure ' ...
    ...
    'and fit a series of logistic mixed effects models with a random intercept for patient. ' ...
    'We prespecified two candidate interactions based on clinical reasoning: ' ...
    'spike rate &times; absolute EEG-visit lag (hypothesizing that predictive value ' ...
    'would attenuate with greater temporal distance between the EEG and clinic visit) ' ...
    'and spike rate &times; lag direction (hypothesizing that predictive value might ' ...
    'differ depending on whether the visit preceded or followed the EEG). ' ...
    'Likelihood ratio tests indicated that the proximity interaction significantly ' ...
    'improved model fit over a main-effects-only model (&chi;&sup2;(1), %s), ' ...
    'while the direction interaction did not improve fit beyond the proximity ' ...
    'interaction alone (&chi;&sup2;(1), %s). ' ...
    'We therefore selected the model including spike rate, absolute lag, their ' ...
    'interaction, lag direction, and epilepsy subtype as fixed effects as our ' ...
    'primary model.</p>\n'], ...
    n_pairs, n_pats, ...
    format_p_html(lrt3_p), ...
    format_p_html(lrt2_p));

fprintf(fid, ['<p>Higher log spike rates were associated with significantly higher odds of ' ...
...
% Model 5: Proximity interaction only, NO epilepsy subtype — for sensitivity
formula_logistic_notype = ['HasSz_bin ~ ' ...
    'LogSpikesPerHour * AbsLag_years + ' ...
    'LagDirection + (1|PatientID)'];

% For no-subtype model, use all patients with valid spike rate, lag, HasSz
keepMask_notype = isfinite(PairTable.LogSpikesPerHour) & ...
                  isfinite(PairTable.SignedLag_years)  & ...
                  isfinite(PairTable.HasSz);

T_notype = PairTable(keepMask_notype, :);
T_notype.HasSz_bin = double(T_notype.HasSz == 1);
T_notype.AbsLag_years = abs(T_notype.SignedLag_years);
T_notype.LagDirection = sign(T_notype.SignedLag_years);
T_notype.LagDirection(T_notype.SignedLag_years == 0) = 1;

fprintf('[No-subtype model] Fitting on %d pairs, %d patients\n', ...
    height(T_notype), numel(unique(T_notype.Patient)));
...
%% ================================================================
%% FIT MODEL 5: Logistic — proximity interaction, no subtype (larger N)
%% ================================================================
fprintf('\nFitting Model 5: Logistic — proximity interaction, no subtype...\n');
try
    mdl_logistic_notype = fitglme(T_notype, formula_logistic_notype, ...
        'Distribution',      'Binomial', ...
        'Link',              'logit', ...
        'FitMethod',         'Laplace', ...
        'CovariancePattern', 'Diagonal');
    fprintf('Model 5 converged successfully.\n');
    disp(mdl_logistic_notype);
catch ME
    disp(['Model 5 failed: ' ME.message]);
    mdl_logistic_notype = [];
end

T_fe5 = [];
if ~isempty(mdl_logistic_notype)
    fprintf('\nModel 5 (Logistic, proximity interaction, no subtype):\n');
    [beta5, betanames5, stats5] = fixedEffects(mdl_logistic_notype, 'Alpha', alpha);
    T_fe5 = table(...
        string(betanames5.Name), beta5, stats5.SE, stats5.tStat, stats5.pValue, ...
        exp(beta5), exp(beta5 - 1.96*stats5.SE), exp(beta5 + 1.96*stats5.SE), ...
...
lrt1_p = NaN; lrt2_p = NaN; lrt3_p = NaN;
...
%% ================================================================
%% BOOTSTRAP CIs — MODEL 5 (no subtype)
%% ================================================================
T_boot5     = [];
boot_betas5 = [];

if ~isempty(mdl_logistic_notype) && nBoot > 0
    fprintf('\nBootstrapping Model 5 (no subtype) CIs (%d iterations)...\n', nBoot);

    patients5   = unique(T_notype.PatientID);
    nPat5       = numel(patients5);
    nFixed5     = size(fixedEffects(mdl_logistic_notype), 1);
    boot_betas5 = nan(nBoot, nFixed5);

    parfor b = 1:nBoot
        idx      = randi(nPat5, nPat5, 1);
        bootPats = patients5(idx);

        Tboot = cell(nPat5, 1);
        for k = 1:nPat5
            Tboot{k} = T_notype(T_notype.PatientID == bootPats(k), :);
            Tboot{k}.PatientID = categorical(repmat(k, height(Tboot{k}), 1));
        end
        Tboot = vertcat(Tboot{:});

        try
            mboot = fitglme(Tboot, formula_logistic_notype, ...
                'Distribution',      'Binomial', ...
                'Link',              'logit', ...
                'FitMethod',         'Laplace', ...
                'CovariancePattern', 'Diagonal');
            boot_betas5(b,:) = fixedEffects(mboot)';
        catch
        end
    end

    converged5  = all(isfinite(boot_betas5), 2);
    boot_betas5 = boot_betas5(converged5, :);
    fprintf('Model 5 bootstrap: %d/%d iterations converged (%.1f%%)\n', ...
        sum(converged5), nBoot, 100*mean(converged5));

    ci_lo5 = prctile(boot_betas5, 100*(alpha/2),   1);
    ci_hi5 = prctile(boot_betas5, 100*(1-alpha/2), 1);

    [beta_obs5, betanames_obs5] = fixedEffects(mdl_logistic_notype);

    T_boot5 = table(...
        string(betanames_obs5.Name), ...
        beta_obs5, ...
        ci_lo5(:), ci_hi5(:), ...
        exp(beta_obs5), exp(ci_lo5(:)), exp(ci_hi5(:)), ...
        'VariableNames', {'Term','Beta','Boot_CI_lo','Boot_CI_hi', ...
                          'OR','OR_CI_lo','OR_CI_hi'});

    fprintf('\nModel 5 bootstrapped ORs and CIs:\n');
    disp(T_boot5);
end
...
MixedModelResults.mdl_logistic_notype    = mdl_logistic_notype;
MixedModelResults.ModelTable_notype      = T_notype;
MixedModelResults.FE_logistic_notype     = T_fe5;
MixedModelResults.BootstrapBetas5        = boot_betas5;
MixedModelResults.BootstrapTable5        = T_boot5;

% LRT stuff
MixedModelResults.LRT1_p = lrt1_p;
MixedModelResults.LRT2_p = lrt2_p;
MixedModelResults.LRT3_p = lrt3_p;
